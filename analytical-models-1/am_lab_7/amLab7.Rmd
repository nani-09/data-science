...
#     Null Hypothesis Ho: Their is no difference in wieghts of the babies of two groupd of smokers
#     Alt  Hypothesis Ha: Their is a difference in wieghts of the babies of two groupd of smokers
...
```{r}
inference(y = nc$weeks, est = "mean", type = "ci", null = 0, 
            alternative = "twosided", method = "simulation", conflevel = 0.90)
```
...
The cuttoff age is in years, either the maximum age of younger momor minium age age of mature mom leads to the results.
...
```{r}
library(gplots)
plotmeans(nc$weeks ~ nc$gender, digits=2, col="red", mean.labels=T, main="Plot of gender means by weeks")
```
...
```
  ...
  From the box plots it is not clearly seen the variations in the means of male and female. So the plotmeans graph clearly shows us that their is a variation in meand of the male and female. there fore we can not reject the null hypothesis.
  The F value is very less,which is kind of bad, the p-value is 0.546 whihc is greater than the 0.056 (as suggested by normal scientific standard) from the ANOVA summary. Hence we can conclude that for our confidence interval we accept the alternative hypothesis H1 that there is no significant relationship between weeks and gender.